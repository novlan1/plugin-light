...
export const babelTransformClass = (
  ...
  for (const result of results) {
    ...
    if (transformed?.code) newSource = newSource.replace(
      ReplaceMarker,
...
export const babelGetVendorRenderPropsExportName = (vendorAsset: OutputChunk) => {
  ...
  for (const [, m] of Object.entries(modules)) {
    const { code } = m;
    if (!code?.includes('renderProps')) continue;
        ...
        visitor: {
          StringLiteral: (path: any) => {
            ...
            if (path.parentPath.parentPath.parentPath.isCallExpression()) {
              ...
              if (
                !options.shouldTransformAttribute(path.parentPath.node.key.value)
              ) return;
            } else {
              if (
              ...
              if (
                !options.shouldTransformAttribute(path.parentPath.parentPath.parentPath.node.key.value)
              ) return;
            }
            const raw = path.node.value;
            const replaced = replaceCharacters(
